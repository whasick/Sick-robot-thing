...
const int ci_Front_Button = 16;// pin A2 used as digital 
...
if (((gauss >80) || (gauss < 40))&& stayout == 0){
 ...
 if (((gauss >80) || (gauss < 40))&& halleffectcount > 5){
  ...
  if ((millis() - Straight_Timer) > 3500)//timer for going straight 
  {
    Straight_Time_Up = true;
  }
   if ((millis() - Spin_Timer) > 5000)//timer for going straight 
  {
    Spin_Time_Up = true;
  }
    if ((millis() - Turn_Timer) < 2000)//timer for turning
...
Ping_Front();
if((digitalRead(13)==HIGH)&&(digitalRead(ci_Front_Button)==LOW))//if it senses pyramid
{
...
if((digitalRead(13)==HIGH)&&(digitalRead(ci_Front_Button)==HIGH))//getting in reach      this will not work w/o encoders (need to sweep)
{
   servo_LeftMotor.writeMicroseconds(1700);//go straight 
   servo_RightMotor.writeMicroseconds(1700);
}

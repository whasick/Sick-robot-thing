...
while(ul_Echo_Time_Front/58 == 0)//attempting to prevent coming off wall
{ 
  Ping_Front();
...
if (((gauss >80) || (gauss < 40))&& stayout == 0){
 ...
 if (((gauss >80) || (gauss < 40))&& halleffectcount > 5){
           ...
           delay(1000);
           servo_PushMotor.writeMicroseconds(1200);
  ...
  if ((millis() - Straight_Timer) > 4000)//timer for going straight 
  {
    Straight_Time_Up = true;
  }
   if ((millis() - Spin_Timer) > 7000)//timer for big spin
  {
    Spin_Time_Up = true;
  }
    if ((millis() - Turn_Timer) < 2000)//timer for turning
   ...
   if ((millis() - Spin_Timer) < 7000)//timer for big spin 
  {
    Spin_Time_Up = false;
        ...
        else if(ul_Echo_Time_Front/58<3)//if it hits a wall, spin NEW******
        {
           servo_LeftMotor.writeMicroseconds(1300);
           servo_RightMotor.writeMicroseconds(1800);
        }
        ...
        if(Straight_Time_Up==false)//straight until straight timer up 
        {
         ...
         if((Straight_Time_Up)&&(Spin_Time_Up==false))    //were going to need to slow down the speed and increase the time to read properly 
        {
          servo_LeftMotor.writeMicroseconds(1300);
           servo_RightMotor.writeMicroseconds(1800);
        ...
        if((Spin_Time_Up)&&(Straight_Time_Up))//if spin time up, reset straight timer 
        {
...
if((Straight_Time_Up)||(digitalRead(ci_Front_Button)==LOW))//if time is up after it sees it 
{
   ...
   delay(4000);
    servo_ArmMotor.writeMicroseconds(ci_Arm_Motor_Stop);
     ...
     delay(1000);
       servo_ArmMotor.writeMicroseconds(ci_Arm_Motor_Stop);
     servo_ClampMotor.write(ci_Clamp_Motor_Open);
      servo_LeftMotor.writeMicroseconds(1300);//go back to get block
   ...
   case 5:    //finished 
      {
        ...
        Serial.println("Mode:5, WE'RE DONE HERE");
        servo_LeftMotor.writeMicroseconds(ci_Left_Motor_Stop);
        servo_RightMotor.writeMicroseconds(ci_Right_Motor_Stop);

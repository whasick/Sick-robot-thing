...
const int ci_Ultrasonic_Side_Back_Data = 6;   //output plug/echo
const int ci_Ultrasonic_Ping_Side_Back = 7;   //input plug/trigger
...
/*
  ...
  Serial.print(ul_Echo_Time_Front / 148); //divide time by 148 to get distance in inches
  Serial.print(", cm: ");
  Serial.println(ul_Echo_Time_Front / 58); //divide time by 58 to get distance in cmm   
  */
...
if((ul_Echo_Time_Front/58>10)&&(abs((ul_Echo_Time_Side_Front/58)-(ul_Echo_Time_Side_Back/58))<=2.5)&&(ul_Echo_Time_Side_Front/58<=6))//if front sensor doesnt see wall
//(within 10cm), the difference between the two sides are within 2.5cm and the front side is within 6cm from the wall
{
   servo_LeftMotor.writeMicroseconds(ui_Left_Motor_Speed);//go straight 
   servo_RightMotor.writeMicroseconds(ui_Right_Motor_Speed);
}
if((ul_Echo_Time_Front/58>7)&&(ul_Echo_Time_Side_Front/58>4))//if front doesnt see a wall and side front is far, go left
{
  servo_LeftMotor.writeMicroseconds(ui_Left_Motor_Speed);
  servo_RightMotor.writeMicroseconds(ci_Right_Motor_Stop);
}
if((ul_Echo_Time_Front/58>7)&&(ul_Echo_Time_Side_Back/58>4))//if front doesnt see a wall and side back is far, go right
{
  servo_LeftMotor.writeMicroseconds(ci_Left_Motor_Stop);
  servo_RightMotor.writeMicroseconds(ui_Right_Motor_Speed);
}
if((ul_Echo_Time_Front/58<7)&&((ul_Echo_Time_Side_Back/58<4)||(ul_Echo_Time_Side_Front/58<4)))//if sees front and either of two others are less then 4
{
  servo_LeftMotor.writeMicroseconds(ci_Left_Motor_Stop);
  servo_RightMotor.writeMicroseconds(ui_Right_Motor_Speed);
}
if((ul_Echo_Time_Front/58>10)&&(ul_Echo_Time_Side_Front/58>6))//if front doesnt see wall and front side is far, go left 
{
  servo_LeftMotor.writeMicroseconds(ui_Left_Motor_Speed);
  servo_RightMotor.writeMicroseconds(ci_Right_Motor_Stop);
}
